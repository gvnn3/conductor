...
class Message:
    ...
    def __init__(self, msg_type: MessageType, payload: Dict[str, Any], version: int = 1):
        self.type = msg_type
...
def step_from_dict(data: Dict[str, Any]):
    ...
    import shlex
    command = shlex.join(data["args"]) if isinstance(data["args"], list) else data["args"]
    return Step(
        command,
        spawn=data.get("spawn", False),
