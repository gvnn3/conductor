...
class TestAudit:
    ...
    def get_test_files(self):
        ...
        return [
            ...
            if f.startswith("test_") and f.endswith(".py") and f != "test_audit.py"
        ]
    ...
    def analyze_test_file(self, filename):
        ...
        if mock_assertions > real_assertions * 2:  # More than 2x mock assertions
            problems.append(
                f"{filename}: Excessive mock assertions ({mock_assertions} mock vs {real_assertions} real)"
            )
        ...
        for method in test_methods:
            ...
            real_assert_pattern = (
                r"\bassert\s+[^_]"  # assert followed by space but not underscore
            )
    ...
    def test_identify_low_quality_tests(self):
        ...
        assert len(found_issues) >= 2, (
            f"Should find at least 2 known issues, found: {found_issues}"
        )
    ...
    def test_placeholder_methods_provide_no_value(self):
        ...
        assert inflation_percentage > 2, (
            "Placeholder tests significantly inflate coverage"
        )
