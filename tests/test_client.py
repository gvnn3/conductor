...
class TestClientCommunication:
    ...
    @patch('socket.create_connection')
    @patch('socket.socket')
    def test_doit_sends_run_command_and_sets_up_results_socket(self, mock_socket_class, mock_create_connection):
        ...
        mock_results_socket.setsockopt.assert_any_call(socket.SOL_SOCKET, socket.SO_REUSEPORT, 1)
...
class TestClientLenRecv:
    ...
    def test_len_recv_handles_partial_reads(self):
        ...
        mock_socket.recv.side_effect = [
            length[:2], length[2:],  # Length header in 2 parts
            test_data[:5], test_data[5:10], test_data[10:]  # Data in 3 parts
        ]
