...
class TestDetailedAudit:
    ...
    def analyze_test_file_ast(self, filename):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                analysis['tests_with_assertions'] += class_analysis['tests_with_assertions']
                analysis['tests_without_assertions'].extend(
                    [f"{class_analysis['name']}.{t}" for t in class_analysis['tests_without_assertions']]
                )
                analysis['placeholder_tests'].extend(
                    [f"{class_analysis['name']}.{t}" for t in class_analysis['placeholder_tests']]
                )
                analysis['mock_heavy_tests'].extend(
                    [f"{class_analysis['name']}.{t}" for t in class_analysis['mock_heavy_tests']]
                )
                analysis['integration_tests'].extend(
                    [f"{class_analysis['name']}.{t}" for t in class_analysis['integration_tests']]
                )
    ...
    def is_placeholder_test(self, func_node, content):
        ...
        placeholder_patterns = [
            'ready()', 'wait_ready()', 'load()',
            'method_exists', 'can_be_called',
    ...
    def is_mock_heavy(self, func_node):
        ...
        for node in ast.walk(func_node):
            if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute):
                ...
                if attr_name in ['assert_called_once_with', 'assert_called_with', 
                               'assert_called_once', 'assert_called', 'call_count']:
                    mock_calls += 1
        ...
        for node in ast.walk(func_node):
            if isinstance(node, ast.Call):
                ...
                if isinstance(node.func, ast.Attribute):
                    if (node.func.attr in ['Popen', 'run', 'check_output'] and
                ...
                if isinstance(node.func, ast.Attribute):
                    if (node.func.attr in ['socket', 'create_connection'] and
                        isinstance(node.func.value, ast.Name) and
                        node.func.value.id == 'socket'):
    ...
    def test_comprehensive_audit(self):
        ...
        total_without_assertions = sum(len(a['tests_without_assertions']) for a in all_analyses)
        ...
        print(f"Tests with assertions: {total_with_assertions} ({total_with_assertions/total_tests*100:.1f}%)")
        print(f"Tests without assertions: {total_without_assertions} ({total_without_assertions/total_tests*100:.1f}%)")
        print(f"Placeholder tests: {total_placeholder} ({total_placeholder/total_tests*100:.1f}%)")
        print(f"Mock-heavy tests: {total_mock_heavy} ({total_mock_heavy/total_tests*100:.1f}%)")
        print(f"Integration tests: {total_integration} ({total_integration/total_tests*100:.1f}%)")
        ...
        for analysis in all_analyses:
            if analysis['tests_without_assertions'] or analysis['placeholder_tests'] or analysis['mock_heavy_tests']:
                print(f"\n{analysis['filename']}:")
                print(f"  Total tests: {analysis['total_tests']}")
                
                if analysis['tests_without_assertions']:
                    print(f"  Tests without assertions: {len(analysis['tests_without_assertions'])}")
                    for test in analysis['tests_without_assertions'][:3]:  # Show first 3
                        print(f"    - {test}")
                    if len(analysis['tests_without_assertions']) > 3:
                        print(f"    ... and {len(analysis['tests_without_assertions']) - 3} more")
                ...
                if analysis['mock_heavy_tests']:
                    ...
                    if len(analysis['mock_heavy_tests']) > 3:
                        print(f"    ... and {len(analysis['mock_heavy_tests']) - 3} more")
        ...
        print(f"This represents approximately {placeholder_coverage/174*100:.1f}% of total coverage")
        ...
        assert total_placeholder >= 3, f"Expected at least 3 placeholder tests, found {total_placeholder}"
