...
class TestRetValProperties:
    ...
    @given(
        code=integers(min_value=0, max_value=65535),
        message=text(max_size=1000)
    )
...
class TestStepProperties:
    ...
    @given(
        command=st.from_regex(r'[a-zA-Z0-9]+ [a-zA-Z0-9\-\./]+ *', fullmatch=True)
    )
...
class TestIntegrationProperties:
    ...
    def test_mixed_command_types(self, spawn_commands, timeout_commands):
        ...
        for i in range(len(spawn_commands), len(spawn_commands) + len(timeout_commands)):
