...
class TestJSONProtocolEdgeCases:
    ...
    @given(
        text=st.one_of(
            ...
            st.text(alphabet="مرحبا世界שלום", min_size=1, max_size=50),  # RTL/International
            st.text(alphabet="\u200b\u200c\u200d\ufeff", min_size=1, max_size=10),  # Zero-width
        )
    )
    ...
    def test_many_keys_in_dictionary(self, num_keys, key_size):
        ...
        data = {
            f"key_{i:0{key_size}d}": f"value_{i}"
    ...
    @given(
        list_size=st.integers(min_value=1000, max_value=5000)
    )
