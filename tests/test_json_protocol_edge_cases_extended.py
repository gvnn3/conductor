...
from conductor.json_protocol import (
    send_message, receive_message, ProtocolError,
    MSG_PHASE, MSG_RESULT, set_max_message_size, get_max_message_size,
    _recv_exactly
)
...
class TestJSONProtocolExtendedEdgeCases:
    ...
    @given(
        partial_length=st.integers(min_value=1, max_value=3)
    )
    def test_receive_partial_length_header(self, partial_length):
        ...
        try:
            ...
            with pytest.raises(ProtocolError, match="Incomplete length header|Connection closed"):
                receive_message(server_sock)
    ...
    @given(
        json_str=st.text(min_size=1).filter(lambda x: not x.strip().startswith('{'))
    )
    def test_receive_invalid_json_format(self, json_str):
        ...
        try:
            ...
            with pytest.raises(ProtocolError, match="Invalid message format|Message must be a JSON object"):
                receive_message(server_sock)
    ...
    def test_receive_unsupported_protocol_version(self, version):
        ...
        try:
            ...
            with pytest.raises(ProtocolError, match=f"Unsupported protocol version: {version}"):
                receive_message(server_sock)
    ...
    def test_special_characters_in_message_data(self, special_chars):
        ...
        data = {
            ...
            "combined": f"text{special_chars}more"
        }
