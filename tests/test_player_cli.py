...
class TestPlayerCLIHelp:
    ...
    def test_help_option(self):
        ...
        result = subprocess.run(
            [sys.executable, "scripts/player", "--help"], capture_output=True, text=True
        )
    ...
    def test_help_short_option(self):
        ...
        result = subprocess.run(
            [sys.executable, "scripts/player", "-h"], capture_output=True, text=True
        )
    ...
    def test_log_file_option(self):
        ...
        try:
            ...
            with open(log_file, "r") as f:
                log_content = f.read()
                assert (
                    "Player started" in log_content or "Player listening" in log_content
                )
...
class TestPlayerCLISignalHandling:
    ...
    def test_port_already_in_use(self):
        ...
        try:
            ...
            assert result.returncode == 1
            assert (
                "Failed to bind" in result.stderr
