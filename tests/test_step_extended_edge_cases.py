...
from conductor.retval import RETVAL_OK


class TestStepExtendedEdgeCases:
    """Extended edge case tests for Step class."""

    @given(command=st.text(min_size=1).filter(lambda x: x.strip() and "\x00" not in x))
    ...
    @given(return_code=st.integers(min_value=1, max_value=255))
    ...
    @given(signals=st.sampled_from(["SIGTERM", "SIGKILL", "SIGINT"]))
